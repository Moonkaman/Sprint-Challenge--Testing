...
describe('gamesRouter.js', () => {
  ...
  describe('POST /api/dogs', () => {
    ...
    it('Should return 201 with the correct information sent', async () => {
      const res = await request(server).post('/api/dogs').send({title: 'Pacman', genre: 'Arcade', releaseYear: 1980});
      expect(res.status).toBe(201);
    });

    it('Should return 422 with incomplete information sent', async () => {
      const res = await request(server).post('/api/dogs').send({title: 'Pacman', genre: 'Arcade'});
    ...
    it('Should return a 400 when trying to add a game thats already there', async () => {
      await request(server).post('/api/dogs').send({title: 'Pacman', genre: 'Arcade', releaseYear: 1980});
      const res = await request(server).post('/api/dogs').send({title: 'Pacman', genre: 'Arcade', releaseYear: 1980});
